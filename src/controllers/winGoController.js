...
const betWinGo = async (req, res) => {
  ...
  console.log("betWinGo", typeid, join, x, money);
  let auth = req.cookies.auth;
    ...
    const sql = `INSERT INTO minutes_1 SET 
        ...
        try {
          ...
          return res.status(500).json({
            message: "Failed to insert into minutes_1 table",
            error: error.message,
            status: false,
          });
        }
...
const addWinGo = async (game) => {
  try {
    ...
    let previousPeriod1 = winGoNow[0].period;
    console.log(Number(previousPeriod1)+1);
    ...
    await connection.query(
      ...
      `,
      [Number(previousPeriod1)+1, join, timeNow],
    );
  } catch (error) {
    if (error) {
      console.log(error);
