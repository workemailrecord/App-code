...
import createUSDTGateway from "../controllers/usdtGatewayController.js"; // Import the createUSDTGateway function
import imageController from '../controllers/imageupload.js';
import multer from 'multer';
import path from 'path';
import { fileURLToPath } from 'url';
// import  {checkWalletBalance} from '../controllers/authenticateAndCheckBalance.js'; 
import  authenticateAndCheckBalance from '../controllers/authenticateAndCheckBalance.js';    
import {updatePopupSetting, getPopupSetting } from '../controllers/popupController.js';  
...
// Storage configuration for 'uploadbanners'
const storageBanners = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, path.join(__dirname, '../public/uploadbanners')); // Navigate to the 'uploadbanners' folder
  },
...
const storageUploads = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, path.join(__dirname, '../public/uploads')); // Navigate to the 'uploads' folder
  },
  filename: (req, file, cb) => {
    cb(null, "uploaded-image" + path.extname(file.originalname)); // Save as 'uploaded-image.<ext>'
  },
});

// Multer upload middleware for each route
const uploadBanners = multer({ storage: storageBanners });
const uploadUploads = multer({ storage: storageUploads });

const initWebRouter = (app) => {
  // Middleware to serve static files
  app.use('/public', express.static(path.join(__dirname, '../src/public'))); // Serve files from public folder 

  // Endpoint for uploading images to 'uploads'
  app.post("/admin/manager/settings/upload-image", uploadUploads.single("image"), (req, res) => {
    try {
      if (req.file) {
        console.log("Image uploaded successfully:", req.file.path); // Log file path
  ...
  router.get('/balancec', authenticateAndCheckBalance);
  // page account
  ...
  router.post("/usdt", createUSDTGateway); // Create the USDT collection order

  router.post("/webhook", accountController.webhook);
  ...
  // BET wingo
  router.get("/wingo", middlewareController,authenticateAndCheckBalance, winGoController.winGoPage);
  ...
  router.get(
    "/trx_wingo",
    middlewareController, authenticateAndCheckBalance,
  ...
  // BET K5D
  router.get("/5d", middlewareController,authenticateAndCheckBalance, k5Controller.K5DPage);
  router.post(
  ...
  // BET K3
  router.get("/k3", middlewareController, authenticateAndCheckBalance, k3Controller.K3Page);
  ...
  router.get('/jslots', middlewareController,homeController.jslotsPage);
  router.get('/alllot', middlewareController,homeController.alllotPage);
  router.get('/allspribe', middlewareController,homeController.allspribePage);
  router.get('/allcasino', middlewareController,homeController.allcasinoPage);
  router.get('/alljdb', middlewareController,homeController.alljdbPage); 
  router.get('/allmancala', middlewareController, homeController.allmancalaPage);
  router.get('/allgtf', middlewareController, homeController.allgtfPage);
  router.get('/allfc', middlewareController, homeController.allfcPage);
  router.get('/allamb', middlewareController, homeController.allambPage);
  router.get('/alljili', middlewareController, homeController.alljiliPage);
  router.get('/allhrg', middlewareController, homeController.allhrgPage);
  router.get('/allonplay', middlewareController, homeController.allonplayPage);
  router.get('/allinjoy', middlewareController, homeController.allinjoyPage);
  router.get('/allswg', middlewareController,homeController.allswgPage);
  router.get('/allyb', middlewareController,homeController.allybPage);
  router.post('/gifthistory', gameController.fetchGiftCodeHistory);
  router.post('/release-vip-level', vipController.releaseVIPLevel);
  ...
  router.get(
    "/admin/manager/statistical4",
    adminController.middlewareAdminController,
    adminController.statistical4,
  ); // get info account
  ...
  router.post(
    "/api/admin/updatePopupSetting",
    adminController.middlewareAdminController, // Ensure admin access
    updatePopupSetting // Use the new controller function
);

// Add a route to get the current popup setting
router.get(
    "/api/admin/getPopupSetting",
    adminController.middlewareAdminController, // Ensure admin access
    getPopupSetting // Use the new controller function
);



  router.post('/upload-images', uploadBanners.fields([
  ...
  router.get('/images', imageController.fetchImageNames);

  router.post('/change-specific-image', imageController.changeSpecificImage);
